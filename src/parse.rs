...
impl Artist {
    ...
    pub(crate) fn parse(res: Value) -> Result<Self, ResponseParseError> {
        Ok(Artist {
            ...
            description: res
                .pointer(ARTIST_DESCIPTION)
                .and_then(|it| Some(it.as_str()?.to_string())),
            albums: iter_from_json(&res, ARTIST_ALBUMS)?
                .filter_map(|item| -> Option<Product> {
                    Some(Product {
                ...
                .collect(),
            singles: iter_from_json(&res, ARTIST_SINGLES)?
                .filter_map(|item| -> Option<Product> {
                    Some(Product {
                        name: string_from_json(&item, ARTIST_PRODUCT_NAME).ok()?,
                        browse_id: string_from_json(&item, ARTIST_PRODUCT_ID).ok()?,
    ...
    pub(crate) fn parse(res: Value) -> Result<Self, ResponseParseError> {
        Ok(Album {
            ...
            tracks: iter_from_json(&res, ALBUM_TRACKS)?
                .enumerate()
                .filter_map(|(track_num, item)| -> Option<Track> {
                    Some(Track {
            ...
            thumbnails: iter_from_json(&res, ALBUM_THUMBS)?
                .filter_map(|thumbnail| -> Option<Thumbnail> {
                    Some(Thumbnail {
                        ...
                        size: (
                            value_from_json(&thumbnail, THUMBNAIL_WIDTH)
                                .ok()?
                                .as_u64()? as usize,
                            value_from_json(&thumbnail, THUMBNAIL_HEIGHT)
                                .ok()?
                                .as_u64()? as usize,
                        ),
                    })
...
impl Ord for Thumbnail {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        self.size
            .0
            .min(self.size.1)
            .cmp(&other.size.0.min(other.size.1))
    }
}
...
impl SongSearchResult {
    ...
    pub(crate) fn parse(res: Value) -> Result<Vec<Self>, ResponseParseError> {
        Ok(iter_from_json(&res, SEARCHED_SONGS)?
            .filter_map(|item| -> Option<Self> {
                Some(Self {
                    name: string_from_json(&item, SEARCHED_SONG_TITLE).ok()?,
                    artist: string_from_json(&item, SEARCHED_SONG_ARTIST).ok()?,
                    album: string_from_json(&item, SEARCHED_SONG_ALBUM).ok()?,
                    duration: string_from_json(&item, SEARCHED_SONG_DURATION).ok()?,
                    thumbnails: iter_from_json(&item, SEARCHED_SONG_THUMBNAILS)
                        .ok()?
                        .filter_map(|thumbnail| {
                            Some(string_from_json(&thumbnail, THUMBNAIL_URL).ok()?)
                        })
                        .collect(),
                    video_id: string_from_json(&item, SEARCHED_SONG_ID).ok()?,
                })
            })
            .collect())
    }
...
fn string_from_json(value: &Value, pointer: &str) -> Result<String, ResponseParseError> {
    match value_from_json(value, pointer)?.as_str() {
        ...
        None => Err(ResponseParseError::BadValue(
            pointer.to_string(),
            value.clone(),
        )),
    }
...
fn iter_from_json<'a>(
    value: &'a Value,
    pointer: &str,
) -> Result<std::slice::Iter<'a, Value>, ResponseParseError> {
...
fn thumbnails_from_json<'a>(
    value: &'a Value,
    pointer: &str,
) -> Result<Vec<Thumbnail>, ResponseParseError> {
    Ok(iter_from_json(value, pointer)?
        .filter_map(|thumbnail| {
            Some(Thumbnail {
                url: string_from_json(&thumbnail, THUMBNAIL_URL).ok()?,
                size: (
...
impl Display for ResponseParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ResponseParseError::MissingValue(path) => {
                write!(f, "Response is missing a value at {path}")
            }
            ResponseParseError::BadValue(path, value) => {
                write!(f, "Response has a bad value at {path}: {value}")
            }
            ResponseParseError::UnclosedConfig(config_text) => write!(
                f,
                "Received response with missing `);` in one of it's configs: {}",
                config_text
            ),
        }
    }
