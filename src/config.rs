...
impl YoutubeConfig {
    pub(crate) fn new(full_config: &HashMap<String, Value>) -> Result<Self, ConfigParseError> {
        Ok(Self {
            ...
            innertube_context_client_name: i64_from_full(
                full_config,
...
fn str_from_full(
    ...
    match full_config.get(key) {
        Some(it) => match it {
            ...
            _ => Err(ConfigParseError::BadConfigParameter {
                key: key.to_string(),
                value: it.clone(),
            }),
        },
        None => Err(ConfigParseError::MissingConfigParameter {
            key: key.to_string(),
        }),
    }
}
...
fn bool_from_full(
    ...
    match full_config.get(key) {
        Some(it) => it.as_bool().ok_or(ConfigParseError::BadConfigParameter {
            key: key.to_string(),
            value: it.clone(),
        }),
        None => Err(ConfigParseError::MissingConfigParameter {
...
pub enum ConfigParseError {
    BadConfigParameter { key: String, value: Value },
    MissingConfigParameter { key: String },
}
